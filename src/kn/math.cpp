...
float increment(float start, float speed, float end) {
    float result = start;
    if (end != 0) {
        if (start > end) result -= speed;
        else if (start < end) result += speed;
    } else {
        if (start > end) result -= speed;
        else if (start < end) result += speed;
    }
    if ((end != 0 && std::abs(start) > std::abs(end)) ||
        (end == 0 && std::abs(start) > std::abs(end) && std::abs(start) < speed)) {
        result = end;
    }
    return result;
}
