...
void polygon(const std::vector<math::Vec2>& shape,const Color& color,const int thickness){
    if (shape.size() > 0){
        for (size_t i = 0; i < shape.size(); ++i) {
            const auto& from = shape[i];
            const auto& to = shape[(i + 1) % shape.size()];  // wraps around to shape[0]
            line(from, to, color, thickness);
        }
    }else point(shape[0],color);
}
